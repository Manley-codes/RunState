...
# Non-functional requirements (July 7, 2026 — updated July 26, 2026; status cells updated July 28, 2026)
...
| 5 | Privacy & consent | Clear separation: local data / sent to AI / sent to weather-music APIs / requires explicit opt-in. | `DATA_PRIVACY.md` live and **reconciled July 28, 2026 with the current Music Intelligence request and the as-built Open-Meteo behavior** (superseding the July 25, 2026 reconciliation) — Anthropic and Open-Meteo disclosures current. Provider/mobile consent tiers remain future release work per the feature map in project_current_state.md. |
...
| 9 | Maintainability & testability | External-service logic isolated from console/history logic (WeatherService pattern); services testable without live APIs. Pays off at the Spring Boot migration. | **Pattern established and expanded:** `ConnectionProvider` seam (DB), `readRuns(Runner, ResultSet)` seam (row decoding), `saveRun`/`buildRunResponse` console delegates (Task 4), plus the Music Intelligence V1 seams — the deterministic no-network `buildRequestBody(Run)` request builder and the package-private `buildFallbackResponse(Run)` used by the music-neutral fallback regression. **198 tests green (July 27, 2026)**, expanded from the **77-test pre-slice baseline (July 25, 2026)**. Hold the pattern. |
...
Source: Codex NFR session July 7, 2026 (items 1, 5–10) + additions (2–4, phase-dependency rule). Updated July 25, 2026 to reflect Pre-Music Integrity Sprint results. Updated July 28, 2026 — status cells 3, 5, and 9 only — to reflect the Music Intelligence V1 prompt slice, the as-built two-request Open-Meteo ceiling, and the reconciled privacy disclosure.
