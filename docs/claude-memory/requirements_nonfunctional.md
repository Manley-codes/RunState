...
| 4 | Reply latency | The finish receives immediate honest confirmation; the contextual reply follows without losing the emotional window. A slow API never blocks saving or leaving the run. | **Console timeouts built:** 5-second connect timeout on the shared HTTP client; 5-second per-request timeout on the Anthropic call; local fallback returns on failure. Future mobile direction: show metrics plus a deterministic factual receipt immediately after durable local save, then request the contextual reflection after a prototype-tuned optional-input window. No fixed delay or measured end-to-end SLO yet. |
...
| 8 | Low-friction reflection | Completing a run is never blocked by reflection. Subjective inputs stay optional and independently skippable; ignored values remain honestly unknown. | The console currently asks sequential post-run energy and effort questions. Mobile shows all three Energy choices at rest and keeps Effort behind quieter `EFFORT +`; every saved run still gets one reflection. Values present when the request starts may inform it; later values save for future patterns and comparisons without regenerating it. Discoverability, exact timing and response quality still need prototype testing. See `design_effort_cost.md`. |
