...
| **Active run session states** — countdown, running, paused, resumed, stopped | Lifecycle, recovery and timestamp contract approved August 25. Android now has the full in-memory ordering, Room-backed start/pause/resume/completion, one UUID-bound active owner, read-only discovery, isolated recovery, a production database and one process-scoped coordinator that orders recovery before countdown/start. MainActivity invokes recovery and shows truthful Ready, Countdown, active, failed and blocked states. Start enters Countdown; Cancel or Android Back returns to Ready without writing. The UI does not yet make the run official or expose active-run controls, and foreground-service execution is not built. Countdown creates no session; Running persists immediately; Paused remains durable; resume is an action; hold-to-end completes. Recovery restores only through the last trustworthy checkpoint. Canonical contract: `run_initiation_register.md`. |
...
**Roadmap position, September 13:** the narrow Core Running Foundation Review, music inventory, and
...
discovery, isolated recovery core, production database, process-scoped admission coordinator and its
first visible entry path. Ready → Countdown → Cancel/Back → Ready now runs through real coordinator
state. The next slice must make the countdown advance into one officially stored run. The
current delivery lane remains the native Android fixture journey:
...
active-row discovery, an isolated recovery core and one process-scoped admission coordinator;
production startup now invokes recovery and the minimal Compose screen renders its admission answer.
