...
| **Active run session states** — countdown, running, paused, resumed, stopped | Lifecycle, recovery and timestamp contract approved August 25. Android now has the full in-memory ordering, Room-backed start/pause/resume/completion, one UUID-bound active owner, read-only discovery, isolated recovery, a production database and one process-scoped coordinator that orders recovery before countdown/start. It remains disconnected from the UI; no Android entry point invokes initialization, and foreground-service execution is not built. Countdown creates no session; Running persists immediately; Paused remains durable; resume is an action; hold-to-end completes. Recovery restores only through the last trustworthy checkpoint. Canonical contract: `run_initiation_register.md`. |
...
discovery, isolated recovery core, production database and process-scoped admission coordinator.
Connecting coordinator initialization to the Android entry path and visible screens is next. The
...
production startup still does not invoke recovery yet.
