...
| **Android fixture journey / Phase 3** — Room version 2, the five-state rules and one UUID-bound `ActiveRunSession` coordinate the durable lifecycle; discovery returns every active candidate and the isolated recovery core handles zero/one/many without guessing, restoring a fresh owner from exactly one Running or Paused row; 69 JVM and 23 emulator tests pass; production database/startup ownership and UI wiring remain | Wire one production database and recovery-before-start ownership boundary | `project_current_state.md` + `run_initiation_register.md` + `design_run_response_system.md` |
