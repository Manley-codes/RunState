...
| **Core Running Foundation Review** — completed; comparison trust is fixed and the mobile lifecycle/recovery/time contract is approved; local-first identity/sync and durable-reflection contracts remain before mobile | Next roadmap lane still needs Manley's choice | `project_current_state.md` + `run_initiation_register.md` |
