...
| **Core Running Foundation Review** — completed; comparison trust gap is fixed, while mobile session/timestamp, local-first identity/sync, and durable-reflection contracts remain before mobile | Comparison fix complete; the next roadmap lane still needs Manley's choice | `project_current_state.md` |
