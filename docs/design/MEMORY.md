...
| **Core Running Foundation Review** — completed; comparison trust is fixed; lifecycle/recovery/time and phone-UUID/local-first-sync contracts are approved; durable selected-reflection remains before mobile | Next roadmap lane still needs Manley's choice | `project_current_state.md` + `run_initiation_register.md` |
