...
| **Android fixture journey / Phase 3** — the Kotlin/Compose shell and five-state rules exist; Room version 2 plus one UUID-bound `ActiveRunSession` coordinate durable and in-memory pause, resume and completion, and read-only active-row discovery now returns every Running or Paused candidate deterministically; 48 JVM and 22 emulator tests pass; recovery and UI wiring remain | Add relaunch recovery before the visible journey | `project_current_state.md` + `run_initiation_register.md` + `design_run_response_system.md` |
