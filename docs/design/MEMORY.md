...
| **Android fixture journey / Phase 3** — Room version 2, the five-state rules and UUID-bound owner coordinate the durable lifecycle; MainActivity now initializes the process-scoped coordinator and renders honest Ready, Countdown, active-run, failure and blocked states; Start enters a non-durable countdown and Cancel/Back exits it safely; 91 JVM and 34 emulator tests pass; no official run is created from the UI yet | Add the timed countdown and official durable start, then active-run controls | `project_current_state.md` + `run_initiation_register.md` + `design_run_response_system.md` |
