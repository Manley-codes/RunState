...
| **Android fixture journey / Phase 3** — foundation started August 26: the Kotlin/Compose shell launches, and an isolated state machine now guards `NO_SESSION → COUNTDOWN → RUNNING ⇄ PAUSED` with twelve passing tests; it is not UI-connected or durable | Continue lifecycle rules in small tested slices, then add the Room-backed Running boundary before the visible journey | `project_current_state.md` + `run_initiation_register.md` + `design_run_response_system.md` |
