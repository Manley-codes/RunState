...
| **Android fixture journey / Phase 3** — the Kotlin/Compose shell and five-state rules exist; Room version 2 now preserves one UUID-backed run through durable pause, resume and completion updates plus ordered transition history, with 40 JVM and 18 emulator tests passing; active-session ownership, recovery and UI wiring remain | Connect the durable data layer to one active-session owner, then add recovery before the visible journey | `project_current_state.md` + `run_initiation_register.md` + `design_run_response_system.md` |
