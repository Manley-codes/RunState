...
| **Android fixture journey / Phase 3** — the Kotlin/Compose shell and five-state rules exist; Room version 1 now saves the UUID-backed initial `RUNNING` row before the in-memory transition, with 35 JVM and 3 emulator tests passing; later transitions, recovery and UI wiring are not durable yet | Extend the same saved run through later lifecycle updates and recovery before the visible journey | `project_current_state.md` + `run_initiation_register.md` + `design_run_response_system.md` |
